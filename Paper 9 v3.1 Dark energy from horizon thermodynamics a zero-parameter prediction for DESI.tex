...
\textit{Series context.}---This paper studies one sector of a broader 
horizon-based, nonlocal effective framework in which a cosmological 
acceleration scale and a retarded memory response emerge from boundary 
information dynamics~\cite{PaperA}. The purpose here is limited: to derive 
and test a single parameter-free prediction for the dark energy equation 
of state against DESI BAO data. Broader claims about the microscopic 
mechanism are deferred to the companion foundational paper.

\textit{Scope of claim.}---This paper does not establish the full 
microscopic theory connecting horizon thermodynamics to the Friedmann 
equation. The derivation of $w(z)$ proceeds from dimensional analysis 
and equilibrium requirements; a microphysical derivation from boundary 
quantum information dynamics remains future work. The result should be 
read as a sector-specific, falsifiable prediction of a horizon-based 
effective framework, not as a complete derivation of dark energy from 
first principles.

\textit{Program architecture.}---This work is part of a broader program 
organized in four layers: (\emph{A})~an empirical low-acceleration 
regularity in gravitating systems; (\emph{B})~an effective nonlocal or 
retarded phenomenology that may account for it; (\emph{C})~a possible 
horizon-based geometric or informational interpretation of that 
phenomenology; and (\emph{D})~a still-incomplete attempt at a unified 
microscopic description. The present paper derives and tests a single 
parameter-free prediction at Layer~C---that the Gibbons-Hawking coupling 
$1/(2\pi)$ determines the dark energy equation of state---against DESI 
DR2 BAO data. Its claims should be read at that level: the $\Delta\chi^2 
= 6.3$ result is a real empirical signal at Layer~A; the $w(z)$ 
prediction is a Layer~C ansatz whose significance warrants but does not 
yet demand the full Layer~D interpretation.
...
$a_u = cH_0/(2\pi) = 1.04\times10^{-10}$~m~s$^{-2}$ marks the 
...
The physical interpretation offered here is that DESI's ``evolving dark energy'' 
signal may be the imprint of horizon thermodynamics on the expansion history, 
rather than a change in the dark energy equation of state. The 
...
\textit{What this paper does not show.}---The compressed CMB likelihood 
used here is an approximation; a definitive significance requires the 
full Planck likelihood within the Cobaya framework. The derivation of 
$w(z)$ is not a microscopic derivation from boundary quantum information 
dynamics --- it is motivated by dimensional analysis and the 
Gibbons-Hawking result. The framework is not tested against supernovae 
or CMB lensing here; those sectors may impose additional constraints. 
The 2.5$\sigma$ significance does not constitute a detection; it is 
evidence of a consistent trend that DESI DR3 will confirm or refute. 
The broader program of which this paper is part remains incomplete at 
...
The author used Anthropic's Claude for manuscript preparation and analytical cross-checking. This work uses public DESI DR2 BAO data from Ref.~\cite{CobayaSampler} and Planck 2018 compressed CMB constraints. DESI is managed by Lawrence Berkeley National Laboratory under U.S. Department of Energy Contract No.\ DE-AC02-05CH11231.
\end{acknowledgments}
...
Zenodo \href{https://doi.org/10.5281/zenodo.19204910}{10.5281/zenodo.19204910} (2026).
...
Zenodo \href{https://doi.org/10.5281/zenodo.19004505}{10.5281/zenodo.19004505} (2026),
